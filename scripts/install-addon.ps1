...
# 此指令稿會自動：
# 1. 偵測您的 Revit 版本
# 2. 從本機複製 RevitMCP.dll 和 RevitMCP.addin
# 3. 複製到正確的資料夾
# ============================================================================
# 安全注意事項：
...
elseif (Test-Path $sourceDllDebug) {
    ...
    Write-Host ""
    Write-Host "或者下載現成版本放到：" -ForegroundColor Yellow
    Write-Host "$projectRoot\MCP\MCP\bin\Release\" -ForegroundColor Yellow
    Read-Host "按 Enter 結束"
    exit 1
}

if (-not (Test-Path $sourceAddin)) {
    Write-Host "❌ 錯誤：找不到 RevitMCP.addin" -ForegroundColor Red
...
try {
    ...
    Write-Host "✓ 已複製 RevitMCP.addin" -ForegroundColor Green
}
catch {
